...
export class UserCoverImageDatabase {
   ...
   */
  public loadCoverImageByUserId = async (userId: number):
      Promise<CoverImage> => {
    ...
    return new CoverImage(parseInt(result.rows[0].user_id),
      result.rows[0].src);
  }

  public uploadCoverImage = async (userId: number,
      imageFile: Express.Multer.File): Promise<CoverImage> => {
