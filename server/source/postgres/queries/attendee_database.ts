...
export class AttendeeDatabase {
  ...
  public loadHomePageEventTagList = async (userId: number): Promise<
    ...
    const result = await this.pool.query(`
      SELECT
        att.event_id, de.color_code, de.title
      FROM
        attendees AS att
      JOIN
        dining_events AS de
      ON
        att.event_id = de.id
      WHERE
        att.user_id = $1 AND att.status = 'GOING' AND de.start_at < NOW() AND
        date_trunc('month', de.start_at) = date_trunc('month', CURRENT_DATE)
      ORDER BY
        de.start_at ASC`, [userId]);
  ...
  public isUserGoingByEventId = async (userId: number, eventId: number):
    ...
    const result = await this.pool.query(`
      ...
      FROM
        attendees
      WHERE
        user_id = $1 AND event_id = $2 AND status = 'GOING'`, [
      userId, eventId]);
  ...
  public joinEvent = async (userId: number, eventId: number): Promise<void> => {
    await this.pool.query(`
      INSERT INTO
        attendees (user_id, event_id, status, guest_count)
      VALUES
        ($1, $2, 'GOING', 0)
      ON CONFLICT
        (user_id, event_id)
      DO UPDATE SET
        status = 'GOING', updated_at = NOW()`, [userId, eventId]);
  }
  ...
  public removeSeat = async (userId: number, eventId: number): Promise<void> =>
    ...
    await this.pool.query(`
      UPDATE
        attendees
      SET
        status = 'NOT_GOING', updated_at = NOW()
      WHERE
        user_id = $1 AND event_id = $2`, [userId, eventId]);
  }
